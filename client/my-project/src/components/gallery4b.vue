...
//
// PURPOSE: SEPERATE THE 4B-SUB-GALLERY PAGE FROM OTHER MODULES, MAKE IT MORE EASILY TO DISPLAY.
//          THIS MODULE IS USED FOR SHOWING ALL DISH PHOTOS OF MORNINGSIDE COLLEGE CAFÉ
//
// STRUCTURE: 
//   (H1,BUTTON, H2) GALLERY HEADER AND BUTTON FOR REDIRECTING TO LAST PAGE
//   (LIGHT-GALLERY) FOR DISPLAY THE PHOTO IN LIGHT-GALLERY
//   (UL) LIST OF DISH PHOTOS OF MORNINGSIDE COLLEGE CAFÉ

<template>
  <div id="gallery4b" class="container-fluid">
    <!-- THIS IS GALLERY HEADER AND BUTTON FOR REDIRECTING TO LAST PAGE -->
    <div>
      <h1>
        Gallery:
        <router-link to="/gallery4"
          ><a class="btn btn-purple" style="color:white;float:right;"
            >Return to Last Page</a
          >
        </router-link>
      </h1>
      <h2>Morningside College Café</h2>
    </div>
    ...
    <!-- THIS IS LIST OF DISH PHOTOS OF MORNINGSIDE COLLEGE CAFÉ -->
...
export default {
  ...
  methods: {
    // METHOD: GETIMAGE
    // PURPOSE: GET ALL DISH PHOTOS OF MORNINGSIDE COLLEGE CAFÉ FROM THE DATABASE  FROM THE DATABASE 
    // INPUT PARAMTER: NONE
    // ALGORITHM: 1. ASK BACKEND TO GET DISH PHOTOS OF MORNINGSIDE COLLEGE CAFÉ FROM THE DATABASE BY SENDING GET COMMAND, AND STORE IT IN DATA
    getImage () {
      this.axios
        .get('http://localhost:3000/api/gallery/' + 'Morningside College Café')
        .then(res => {
