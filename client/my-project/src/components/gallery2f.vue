...
//
// PURPOSE: SEPERATE THE 2F-SUB-GALLERY PAGE FROM OTHER MODULES, MAKE IT MORE EASILY TO DISPLAY.
//          THIS MODULE IS USED FOR SHOWING ALL DISH PHOTOS OF BASIC MEDICAL SCIENCES BUILDING SNACK BAR
//
// STRUCTURE: 
//   (H1,BUTTON, H2) GALLERY HEADER AND BUTTON FOR REDIRECTING TO LAST PAGE
//   (LIGHT-GALLERY) FOR DISPLAY THE PHOTO IN LIGHT-GALLERY
//   (UL) LIST OF DISH PHOTOS OF BASIC MEDICAL SCIENCES BUILDING SNACK BAR

<template>
  <div id="gallery2f" class="container-fluid">
    ...
    <!-- THIS IS LIST OF DISH PHOTOS OF BASIC MEDICAL SCIENCES BUILDING SNACK BAR -->
...
export default {
  ...
  methods: {
    // METHOD: GETIMAGE
    // PURPOSE: GET ALL DISH PHOTOS OF BASIC MEDICAL SCIENCES BUILDING SNACK BAR FROM THE DATABASE  FROM THE DATABASE 
    // INPUT PARAMTER: NONE
    // ALGORITHM: 1. ASK BACKEND TO GET DISH PHOTOS OF BASIC MEDICAL SCIENCES BUILDING SNACK BAR FROM THE DATABASE BY SENDING GET COMMAND, AND STORE IT IN DATA
    getImage () {
      this.axios
        .get('http://localhost:3000/api/gallery/' + 'Orchid Lodge')
        .then(res => {
