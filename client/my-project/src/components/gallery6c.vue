...
//
// PURPOSE: SEPERATE THE 6C-SUB-GALLERY PAGE FROM OTHER MODULES, MAKE IT MORE EASILY TO DISPLAY.
//          THIS MODULE IS USED FOR SHOWING ALL DISH PHOTOS OF NEW ASIA COLLEGE YUN CHI HSIEN (FOR MEMBERS ONLY)
//
// STRUCTURE: 
//   (H1,BUTTON, H2) GALLERY HEADER AND BUTTON FOR REDIRECTING TO LAST PAGE
//   (LIGHT-GALLERY) FOR DISPLAY THE PHOTO IN LIGHT-GALLERY
//   (UL) LIST OF DISH PHOTOS OF NEW ASIA COLLEGE YUN CHI HSIEN (FOR MEMBERS ONLY)

<template>
  <div id="gallery6c" class="container-fluid">
    ...
    <div>
      ...
      <h2>New Asia College Yun Chi Hsien (For members only)</h2>
    </div>
    ...
    <!-- THIS IS LIST OF DISH PHOTOS OF NEW ASIA COLLEGE YUN CHI HSIEN (FOR MEMBERS ONLY) -->
...
export default {
  ...
  methods: {
    // METHOD: GETIMAGE
    // PURPOSE: GET ALL DISH PHOTOS OF NEW ASIA COLLEGE YUN CHI HSIEN (FOR MEMBERS ONLY) FROM THE DATABASE  FROM THE DATABASE 
    // INPUT PARAMTER: NONE
    // ALGORITHM: 1. ASK BACKEND TO GET DISH PHOTOS OF NEW ASIA COLLEGE YUN CHI HSIEN (FOR MEMBERS ONLY) FROM THE DATABASE BY SENDING GET COMMAND, AND STORE IT IN DATA
    getImage () {
      this.axios
        .get(
          'http://localhost:3000/api/gallery/' +
            'New Asia College Yun Chi Hsien (For members only)'
        )
        .then(res => {
